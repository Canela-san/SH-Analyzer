...
;
; IMPORTANTE: o comando é carregado já alinhado na parte ALTA do registrador
; (0xC0000000 em vez de 0x0000C000), ou seja, o bit mais significativo do
; comando fica no bit 31 do registrador. Isso permite extrair cada bit usando
; só deslocamentos e uma máscara pequena (o clpru só aceita imediato de até
; 8 bits em instruções como AND/OR - uma máscara de 0x8000 não é aceita).
;
; ==============================================================================
...
; ==============================================================================
CMD_BIT .macro
    LSR r29, r28, 30        ; 1. Traz os bits [31:30] para as posições [1:0]
    AND r29, r29, 2         ; 2. Isola só o bit de interesse (posição do SDI = Bit 1)
    CLR r30, r30, 1         ; 3. Zera o pino SDI
    OR  r30, r30, r29       ; 4. Escreve o bit de comando no SDI (branchless)
    LSL r28, r28, 1         ; 5. Desloca o comando (prepara o próximo bit)
...
espera_configuracao:
    ...
    ; OBS: o clpru não aceita "MOV Rd, imediato" (MOV só existe entre dois
    ; registradores); para carregar uma constante use sempre LDI.
    MOV r19, r24          ; r19 = Ponteiro Físico Atual (Começa apontando para o Buffer 0)
    LDI r15, 0             ; r15 = Contador de amostras no buffer atual
    LDI r26, 0             ; r26 = active_buffer_flag (0 = Buffer 0, 1 = Buffer 1)
...
wait_time:
    ...
    LDI r23, 0               ; Zera o acumulador de dados
    LDI r28.w0, 0x0000       ; Carrega o comando MAN_Ch_0 (0xC0000000) já alinhado
    LDI r28.w2, 0xC000       ; na parte alta do registrador (ver comentário acima)
    ...
    LDI r15, 0               ; Zera o contador de amostras
...
troca_para_buffer_0:
    ...
    LDI r26, 0               ; Atualiza flag ativa para Buffer 0
...
troca_para_buffer_1:
    ...
    LDI r26, 1               ; Atualiza flag ativa para Buffer 1
